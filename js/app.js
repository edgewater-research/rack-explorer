...
function renderInspector(){
  ...
  if (kind==='xpu'){
    ...
    h += head('Management port', `BMC 1 GbE · ${b.rack.mgmt_ports_per_tray ?? 1} per tray`);
    h += `<div class="sec">${kv([['Per rack', `${(b.rack.mgmt_ports_per_tray ?? 1) * b.rack.compute_trays} compute + ${b.rack.switch_trays} switch trays`],['Goes to', 'Out-of-band management switch (1 GbE)'],['Media', 'Cat6 / RJ45']])}<div class="form" style="margin-top:8px">${inp('BMC ports per tray','rack.mgmt_ports_per_tray',{step:1})}</div></div>`;
  } else if (kind==='drives'){
    h += head('E1.S NVMe bays', `${b.rack.drives_per_tray ?? 0} per tray · local storage`);
    h += `<div class="sec">${kv([['Per rack', `${(b.rack.drives_per_tray ?? 0) * b.rack.compute_trays}`],['Network', 'None — attached over PCIe to the host / DPU']])}<div class="form" style="margin-top:8px">${inp('Drive bays per tray','rack.drives_per_tray',{step:1})}</div></div>`;
  } else if (kind==='leaf'){
...
function renderBuild(){
  ...
  <div class="sec"><h4>Building blocks</h4><div class="form">${inp('XPU','xpu',{select:opt(D.XPUS)})}${inp('Host CPU','cpu',{select:opt(D.CPUS)})}${inp('Scale-out NIC','nic',{select:opt(D.NICS)})}${inp('NICs per XPU','rack.nics_per_xpu',{step:'any'})}${inp('DPU','dpu',{select:opt(D.DPUS)})}${inp('DPUs per tray','rack.dpus_per_tray',{step:1})}${inp('E1.S drive bays per tray','rack.drives_per_tray',{step:1})}${inp('BMC ports per tray','rack.mgmt_ports_per_tray',{step:1})}</div><div class="hint" style="padding:6px 0 0">Pick from the library on the left or here; edit a block's specs from the Inspector after clicking it in the drawing.</div></div>
